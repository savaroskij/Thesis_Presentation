...
\begin{frame}
\frametitle{Temperature and Polarization Anisotropies}
...
\begin{columns}
        ...
        \begin{column}{0.38\textwidth}
        \scriptsize
                \begin{block}{Temperature/Polarization Power Spectra}
                        \tiny
                        \begin{align}
...
\section{The Search for B-modes}
...
\begin{columns}
        \begin{column}{0.40\textwidth}
                ...
                \alert{Prediction:}\\
                \vspace{0.3cm}
                \includegraphics[width=1\textwidth]{predicted_power_spectra}
                \vspace{0.25cm}
        \end{column}
        \only<1-2>{
                \onslide<2>{
        ...
        \only<3>{
                \begin{column}{0.605\textwidth}
                        \centering
                        \alert{State of the art:}\\
                        \includegraphics[width=1\textwidth]{CMB-S4_polarization_status}
                \end{column}
        }
\end{columns}
...
\begin{columns}
        \begin{column}{0.38\textwidth}
                \includegraphics[width=0.48\textwidth]{lspe}
                \includegraphics[width=0.48\textwidth]{Logo-ASI-2019}
                \begin{itemize}
                        \item<2-> \alert{Upper limit} to B-modes intensity
                        \item<3-> \alert{Strip} $\rightarrow$ Observatorio del
                        Teide 
                \end{itemize}
                \onslide<4->{
        ...
        \onslide<3->{
                \begin{column}{0.58\textwidth}
                        \includegraphics<3>[width=1\textwidth]{observatorio_teide}
                        \includegraphics<4>[width=1\textwidth]{strip-focal-plane_2}
                \end{column}
        }
\end{columns}
...
\begin{columns}
        \begin{column}{0.35\textwidth}
                ...
                \small
                \begin{itemize}
                        \item<2-> Almost \alert{unpolirized} \large
...
\section{Atmopsheric Emission Model}
...
\only<6-7>{
\begin{block}{Kirchhoff Law \onslide<7->{\textcolor{red}{(Valid for system at LTE e.g ELA)}}}<6-7>
\begin{equation}
B_\nu(T) = \frac{j_\nu(T)}{\alpha_\nu(T)}
\end{equation}
\end{block}
...
\only<8->{%
\vspace{0.4cm}%
\begin{block}{Atmospheric Brightness Temperature}<8->%
\only<8-9>{%
\begin{equation}%
T_{sky}(\nu) = \textcolor<9>{red}{T_{atm}^{ph}(1 - e^{-\tau_A(\nu)})} + T_{CMB}(\nu)e^{-\tau_A(\nu)}%
\end{equation}%
\vspace{0.05cm}%
}%
\only<10->{%
\begin{equation}%
\textcolor{red}{T_{atm}(\nu)} = T_{sky}(\nu) - T_{CMB}(\nu)e^{-\tau_A(\nu)}%
\onslide<11>{\rightarrow \textit{\alert{CMB Atmospheric Library}}}%
\end{equation}%
}%
\end{block}%
}%
