...
\centering
\LARGE
\alert{\textit{Smoking gun} inflationary paradigm}
}
...
\item<2-3,5-> \only<2-5>{$\epsilon_R \propto \sqrt{n_\nu}$}\only<6->{\sout{$\epsilon_R \propto \sqrt{n_\nu}$}},
...
\begin{block}{Radiative Transfer Equation}<7->
\begin{equation}
\frac{dI_\nu}{ds} = -\alpha_\nu I_\nu + j_\nu
\end{equation}
\end{block}

\begin{block}{Kirchhoff Law \onslide<9->{\textcolor{red}{(Valid for system at LTE e.g ELA)}}}<8->
\begin{equation}
B_\nu(T) = \frac{j_\nu(T)}{\alpha_\nu(T)}
\end{equation}
\end{block}
...
Integrating from ground to \alert{tropopause} and applaying RJ approximation:
...
\onslide<2->{Numerical approach:}
\begin{itemize}
\item<3-> Atmosphere discretization into \alert{layers}
\item<4-> Write radiative transfer equation for each layer \onslide<6->{$\rightarrow$ \textcolor{red}{input?}}
\item<5-> Solve this system of equations
\end{itemize}
